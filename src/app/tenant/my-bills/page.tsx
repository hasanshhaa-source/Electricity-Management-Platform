import { requireTenant } from '@/services/auth/authService';
import { getTenancyByUser } from '@/services/tenant/tenancyService';
import { getTenantBillsWithPayments } from '@/services/billing/paymentService';
import { getTenantInsights } from '@/services/insights/insightService';
import { PageHeader } from '@/components/shared/page-header';
import { EmptyState } from '@/components/shared/empty-state';
import { Alert, AlertDescription, AlertTitle } from '@/components/ui/alert';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { BillStatusBadge } from '@/components/shared/status-badge';
import { TenantConsumptionChart } from './consumption-chart';
import { BillCard } from './bill-card';
import {
  FileText, AlertTriangle, CheckCircle2, TrendingUp, TrendingDown,
  Zap, DollarSign, Clock, Award,
} from 'lucide-react';

const MONTH_NAMES = [
  'January','February','March','April','May','June',
  'July','August','September','October','November','December',
];

function fmt(n: number, d = 2) {
  return n.toLocaleString('en', { minimumFractionDigits: d, maximumFractionDigits: d });
}

export default async function MyBillsPage() {
  const user = await requireTenant();
  const { data: tenancy } = await getTenancyByUser(user.id);

  if (!tenancy || tenancy.status !== 'active') {
    return (
      <div className="space-y-6">
        <PageHeader title="Bill History" description="Your electricity bills" />
        <Alert variant="warning">
          <AlertTriangle className="h-4 w-4" />
          <AlertTitle>No active tenancy</AlertTitle>
          <AlertDescription>You need an approved flat assignment to view bills.</AlertDescription>
        </Alert>
      </div>
    );
  }

  const [bills, insights] = await Promise.all([
    getTenantBillsWithPayments(tenancy.flat_id),
    getTenantInsights(tenancy.flat_id),
  ]);

  const { currency } = insights;
  const outstandingBills = bills.filter(b => ['unpaid','partial','overdue'].includes(b.status));
  const settledBills     = bills.filter(b => ['paid','waived'].includes(b.status));
  const totalOutstanding = outstandingBills.reduce((s, b) => s + b.outstandingBalance, 0);
  const hasOverdue       = outstandingBills.some(b => b.status === 'overdue');

  // Trend comparison table — last 6 bills
  const trendBills = bills.slice(0, 6);

  // Current vs previous month comparison
  const curr = bills[0];
  const prev = bills[1];
  const consumptionChange = curr && prev && prev.billedUnits > 0
    ? ((curr.billedUnits - prev.billedUnits) / prev.billedUnits) * 100 : null;

  return (
    <div className="space-y-6">
      <PageHeader
        title="Bill History"
        description={`Electricity bills for Flat ${tenancy.flat?.flat_number}`}
      />

      {bills.length === 0 ? (
        <EmptyState
          icon={<FileText className="h-8 w-8" />}
          title="No bills yet"
          description="Bills will appear here once your admin generates them for the billing period"
        />
      ) : (
        <>
          {/* Outstanding / all-clear banner */}
          {totalOutstanding > 0 ? (
            <div className={`rounded-lg border p-4 ${hasOverdue ? 'border-red-200 bg-red-50' : 'border-amber-100 bg-amber-50'}`}>
              <div className="flex items-start gap-3">
                <AlertTriangle className={`h-5 w-5 mt-0.5 shrink-0 ${hasOverdue ? 'text-red-500' : 'text-amber-500'}`} />
                <div>
                  <p className={`font-semibold ${hasOverdue ? 'text-red-700' : 'text-amber-700'}`}>
                    Outstanding: {currency} {fmt(totalOutstanding)}
                  </p>
                  <p className={`text-sm mt-0.5 ${hasOverdue ? 'text-red-600' : 'text-amber-600'}`}>
                    {hasOverdue ? 'Overdue bills exist. Please contact your admin.' : 'Please pay before the due date.'}
                  </p>
                </div>
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-2 rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-700">
              <CheckCircle2 className="h-4 w-4 shrink-0" />
              All bills paid. Great job!
            </div>
          )}

          {/* ── Insights cards ─────────────────────────────────────── */}
          {insights.totalBills > 0 && (
            <section>
              <h2 className="text-sm font-semibold text-gray-700 uppercase tracking-wide mb-3">My Insights</h2>
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                <div className="rounded-lg border border-gray-200 bg-white p-3">
                  <div className="flex items-center gap-1.5 mb-1">
                    <Zap className="h-3.5 w-3.5 text-yellow-500" />
                    <span className="text-xs text-gray-500">Avg Consumption</span>
                  </div>
                  <p className="text-lg font-bold text-gray-900">{fmt(insights.avgMonthlyConsumption, 1)} kWh</p>
                  <p className="text-xs text-gray-400">per month</p>
                </div>

                <div className="rounded-lg border border-gray-200 bg-white p-3">
                  <div className="flex items-center gap-1.5 mb-1">
                    <DollarSign className="h-3.5 w-3.5 text-blue-500" />
                    <span className="text-xs text-gray-500">Avg Bill</span>
                  </div>
                  <p className="text-lg font-bold text-gray-900">{currency} {fmt(insights.avgMonthlyBill)}</p>
                  <p className="text-xs text-gray-400">per month</p>
                </div>

                <div className="rounded-lg border border-gray-200 bg-white p-3">
                  <div className="flex items-center gap-1.5 mb-1">
                    <Clock className="h-3.5 w-3.5 text-indigo-500" />
                    <span className="text-xs text-gray-500">On-Time Rate</span>
                  </div>
                  <p className={`text-lg font-bold ${insights.onTimeRate >= 80 ? 'text-green-700' : insights.onTimeRate >= 50 ? 'text-orange-500' : 'text-red-600'}`}>
                    {fmt(insights.onTimeRate, 0)}%
                  </p>
                  <p className="text-xs text-gray-400">paid before due date</p>
                </div>

                <div className="rounded-lg border border-gray-200 bg-white p-3">
                  <div className="flex items-center gap-1.5 mb-1">
                    <Award className="h-3.5 w-3.5 text-green-500" />
                    <span className="text-xs text-gray-500">Total Paid</span>
                  </div>
                  <p className="text-lg font-bold text-green-700">{currency} {fmt(insights.totalPaid)}</p>
                  <p className="text-xs text-gray-400">across {insights.totalBills} bills</p>
                </div>
              </div>

              {/* Current vs previous month highlight */}
              {consumptionChange !== null && (
                <div className={`mt-3 rounded-lg border p-3 text-sm flex items-center gap-3
                  ${Math.abs(consumptionChange) >= 30 ? 'border-orange-200 bg-orange-50' : 'border-gray-100 bg-gray-50'}`}>
                  {consumptionChange > 0
                    ? <TrendingUp className="h-4 w-4 text-red-500 shrink-0" />
                    : <TrendingDown className="h-4 w-4 text-green-600 shrink-0" />}
                  <span className="text-gray-700">
                    This month's consumption is{' '}
                    <strong className={consumptionChange > 0 ? 'text-red-600' : 'text-green-700'}>
                      {consumptionChange > 0 ? '+' : ''}{consumptionChange.toFixed(1)}%
                    </strong>{' '}
                    compared to last month
                    {Math.abs(consumptionChange) >= 30 ? ' — significant change detected.' : '.'}
                  </span>
                </div>
              )}
            </section>
          )}

          {/* ── 12-Month trend chart ───────────────────────────────── */}
          {insights.yearlyTrend.some(t => t.consumption > 0) && (
            <Card>
              <CardHeader>
                <CardTitle className="text-sm flex items-center gap-2">
                  <TrendingUp className="h-4 w-4 text-indigo-500" />
                  12-Month Consumption &amp; Cost Trend
                </CardTitle>
              </CardHeader>
              <CardContent>
                <TenantConsumptionChart data={insights.yearlyTrend} currency={currency} />
              </CardContent>
            </Card>
          )}

          {/* ── Trend table ────────────────────────────────────────── */}
          {trendBills.length >= 2 && (
            <section>
              <h2 className="text-sm font-semibold text-gray-700 uppercase tracking-wide mb-3">
                Month-by-Month Trend
              </h2>
              <div className="overflow-x-auto rounded-lg border border-gray-200 bg-white">
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 text-xs text-gray-500">
                    <tr>
                      <th className="text-left px-4 py-2.5 font-medium">Period</th>
                      <th className="text-right px-4 py-2.5 font-medium">kWh</th>
                      <th className="text-right px-4 py-2.5 font-medium">vs Prior</th>
                      <th className="text-right px-4 py-2.5 font-medium">Total ({currency})</th>
                      <th className="text-right px-4 py-2.5 font-medium">vs Prior</th>
                      <th className="px-4 py-2.5 font-medium">Status</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50">
                    {trendBills.map((bill, idx) => {
                      const p2         = trendBills[idx + 1];
                      const kwdDiff    = p2 ? bill.billedUnits - p2.billedUnits : null;
                      const amtDiff    = p2 ? bill.totalDue    - p2.totalDue    : null;
                      const kwdPct     = p2 && p2.billedUnits > 0 ? (kwdDiff! / p2.billedUnits) * 100 : null;
                      const amtPct     = p2 && p2.totalDue    > 0 ? (amtDiff!  / p2.totalDue)  * 100 : null;
                      const isAnomaly  = kwdPct !== null && Math.abs(kwdPct) >= 30;
                      return (
                        <tr key={bill.id} className={`hover:bg-gray-50 ${isAnomaly ? 'bg-orange-50/50' : ''}`}>
                          <td className="px-4 py-2.5 font-medium text-gray-900">
                            {MONTH_NAMES[bill.periodMonth - 1]} {bill.periodYear}
                          </td>
                          <td className="px-4 py-2.5 text-right font-mono">{fmt(bill.billedUnits, 1)}</td>
                          <td className="px-4 py-2.5 text-right">
                            {kwdPct !== null ? (
                              <span className={`inline-flex items-center gap-0.5 text-xs font-medium ${kwdPct > 0 ? 'text-red-500' : kwdPct < 0 ? 'text-green-600' : 'text-gray-400'}`}>
                                {kwdPct > 0 ? <TrendingUp className="h-3 w-3" /> : kwdPct < 0 ? <TrendingDown className="h-3 w-3" /> : null}
                                {kwdPct !== 0 ? `${kwdPct > 0 ? '+' : ''}${kwdPct.toFixed(1)}%` : '—'}
                                {isAnomaly && kwdPct > 0 ? ' ⚠' : ''}
                              </span>
                            ) : <span className="text-xs text-gray-300">—</span>}
                          </td>
                          <td className="px-4 py-2.5 text-right font-mono">{fmt(bill.totalDue)}</td>
                          <td className="px-4 py-2.5 text-right">
                            {amtPct !== null ? (
                              <span className={`inline-flex items-center gap-0.5 text-xs font-medium ${amtPct > 0 ? 'text-red-500' : amtPct < 0 ? 'text-green-600' : 'text-gray-400'}`}>
                                {amtPct > 0 ? <TrendingUp className="h-3 w-3" /> : amtPct < 0 ? <TrendingDown className="h-3 w-3" /> : null}
                                {amtPct !== 0 ? `${amtPct > 0 ? '+' : ''}${amtPct.toFixed(1)}%` : '—'}
                              </span>
                            ) : <span className="text-xs text-gray-300">—</span>}
                          </td>
                          <td className="px-4 py-2.5">
                            <BillStatusBadge status={bill.status as any} />
                          </td>
                        </tr>
                      );
                    })}
                  </tbody>
                </table>
              </div>
            </section>
          )}

          {/* Outstanding bills */}
          {outstandingBills.length > 0 && (
            <section className="space-y-3">
              <h2 className="text-sm font-semibold text-gray-700 uppercase tracking-wide">
                Outstanding Bills ({outstandingBills.length})
              </h2>
              {outstandingBills.map(bill => (
                <BillCard key={bill.id} bill={bill} currency={currency} />
              ))}
            </section>
          )}

          {/* Payment history */}
          {settledBills.length > 0 && (
            <section className="space-y-3">
              <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide">
                Payment History ({settledBills.length})
              </h2>
              {settledBills.map(bill => (
                <BillCard key={bill.id} bill={bill} currency={currency} />
              ))}
            </section>
          )}
        </>
      )}
    </div>
  );
}
